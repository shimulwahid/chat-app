...
function App() {
  ...
  useEffect(() => {
    ...
    const SOCKET_URL = "https://chat-app-ty7z.onrender.com"//|| "http://localhost:5000";

    const newSocket = io(SOCKET_URL, {
      transports: ["websocket"], // force persistent WebSocket
    });
  ...
  const sendMessage = (e) => {
    ...
    if (privateTo) {
      ...
      setMessages((prev) => [
        ...prev,
        { user: "You (private)", text: message, time: new Date().toLocaleTimeString(), private: true },
      ]);
    } else {
        ...
        <>
          ...
          <div className="users-list">
            ...
            {privateTo && (
              <div style={{ color: "red", marginTop: "5px" }}>
                ...
                <button style={{ marginLeft: "10px" }} onClick={() => setPrivateTo(null)}>
                  Back
                </button>
              </div>
            )}
          ...
          <div className="messages">
              ...
              return (
                <div key={i} className={`message ${isMe ? "my-message" : "other-message"}`}>
                  <div className={`bubble ${isMe ? "my-bubble" : "other-bubble"}`}>
                    ...
                    <div>
                      {msg.private && <b style={{ color: "red" }}>[PRIVATE]</b>} {msg.text}
                    </div>
                  </div>
                </div>
              );
            ...
            {typingUser && typingUser !== username && (
              <div style={{ fontSize: "12px", opacity: 0.7 }}>{typingUser} is typing...</div>
            )}
